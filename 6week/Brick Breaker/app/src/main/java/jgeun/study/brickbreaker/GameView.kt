...
class GameView : View {
    ...
    private var screenLineTop = 250F
    private var screenGameTop = 260F
    private var life = 3

    private var score = 0
    private var highScore = 1200
    ...
    private var effectThreadArray : ArrayList<EffectThread> = ArrayList()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        //canvas.drawBitmap(barImage, bar.x, bar.y, null)
    }
                ...
                while(true) {
                    if (!stopFlag) {
                        ...
                        if (ball.x < ball.width / 2) {                           // 왼쪽 벽
                            ...
                            if(life == 0){
                                Log.d("Life", "life 0이라 들어옴")
                                blockThread.interrupt()
                                (context as Activity).startActivity(Intent(context, GameOverActivity::class.java))
                    ...
                    for(ball in ballArray) {
                        if (block.isExist && block.isBallHit(ball)) {
                            ...
                            if (blockCount <= 0) {
                                ...
                                (context as Activity).startActivity(Intent(context, GameClearActivity::class.java))
                                (context as Activity).finish()
                            }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if(event!!.action == MotionEvent.ACTION_DOWN){
            if(event.x >= 950 && event.x <= 1050 && event.y >= 40 && event.y <=140) {
                ...
                for(ballThread in ballThreadArray)
                    ballThread.stopFlag = true
                //(context as Activity).setContentView(GameView())
                (context as Activity).startActivity(Intent(context, PauseActivity::class.java))
                (context as Activity).finish()
            }
            barX = event.getX()
